...
def test_cover_complex():
    ...
    F = np.array(
        [
            [1, 1, 1, 1, 1, 1.5, 1.5, 2, 2, 2, 2, 2],
            [1, 1.5, 2, 2.5, 3, 2, 3, 2, 2.5, 3, 3.5, 4],
        ]
    ).T

    M = GraphInducedComplex(
        input_type="point cloud",
        cover="functional",
        min_points_per_node=0,
        filter_bnds=np.array([0.5, 4.5]),
        resolution=4,
        gain=0.3,
        graph="rips",
        rips_threshold=0.6,
        verbose=True,
    ).fit(X, filter=F[:, 1], color=None)

    assert list(M.simplex_tree_.get_filtration()) == [
        ([0], 0.0),
        ([1], 0.0),
        ([0, 1], 0.0),
        ([2], 0.0),
    ...
    M = MapperComplex(
        ...
        clustering=AgglomerativeClustering(
            n_clusters=None, linkage="single", distance_threshold=0.6
        ),
    ).fit(X, filters=F, colors=None)
    ...
    M = NerveComplex(input_type="point cloud", min_points_per_node=0).fit(
        X,
        assignments=[
            [0],
            [0, 1],
